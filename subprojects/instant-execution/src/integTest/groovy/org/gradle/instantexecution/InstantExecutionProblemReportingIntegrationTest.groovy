...
class InstantExecutionProblemReportingIntegrationTest extends AbstractInstantExecutionIntegrationTest {
    ...
    def "reports problems in project build scripts"() {
        ...
        then:
        problems.assertFailureHasProblems(failure) {
            withProblem("build file 'a/build.gradle': registration of listener on 'Gradle.buildFinished' is unsupported")
            withProblem("build file 'a/build.gradle': invocation of 'Task.project' at execution time is unsupported.")
        }
    }
    ...
    def "reports problems in project Kotlin build scripts"() {
        ...
        then:
        problems.assertFailureHasProblems(failure) {
            withProblem("build file 'a/build.gradle.kts': registration of listener on 'Gradle.buildFinished' is unsupported")
